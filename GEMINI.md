...
Instruções para o Gemini trabalhando neste repositório.
...
- Execute somente o que foi explicitamente pedido.
...
- Não exponha stack traces internos ao cliente em produção.

## Migrations

- Gerenciadas pelo Prisma Migrate.
- Schema em `prisma/schema.prisma`.
- Para criar nova migration: `pnpm db:migrate:dev`.
- Dados de seed em `prisma/seed-data/` e executados via `prisma/seed.ts`.
- Use `include` vs `select` com cuidado para evitar over-fetching.

## Background Jobs (BullMQ)

- Tokens de filas e tarefas em `src/lib/bullmq/helpers/bullmq-tokens.ts`.
- Separação entre **Processor** (lógica) e **Queue** (definição da fila).
- Facades em `src/jobs/queues/facades/`.
- Cron schedulers em `src/jobs/cron/schedulers/`.
