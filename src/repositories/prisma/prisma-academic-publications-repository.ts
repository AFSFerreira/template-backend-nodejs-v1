...
export class PrismaAcademicPublicationsRepository implements AcademicPublicationsRepository {
  async create(data: Prisma.AcademicPublicationsCreateInput) {
    const academicPublication = await prisma.academicPublications.create({ data })
    return academicPublication
  }

  async findById(id: string) {
    const academicPublication = await prisma.academicPublications.findUnique({ where: { id } })
    return academicPublication
  }

  async findManyByUserId(userId: string) {
    const academicPublications = await prisma.academicPublications.findMany({ where: { userId } })
  ...
  async update(id: string, data: Prisma.AcademicPublicationsUpdateInput) {
    const academicPublication = await prisma.academicPublications.update({ where: { id }, data })
    return academicPublication
  }
}
