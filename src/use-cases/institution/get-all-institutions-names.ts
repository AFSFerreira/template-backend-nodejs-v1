...
export class GetAllInstitutionsNamesUseCase {
  ...
  async execute(getAllInstitutionsUseCaseInput: GetAllInstitutionsNamesUseCaseRequest): Promise<GetAllInstitutionsNamesUseCaseResponse> {
    ...
    const filteredInstitutions = universityName ? allInstitutions.filter((institution) => institution.name.toUpperCase().trim().includes(universityName)) : allInstitutions
