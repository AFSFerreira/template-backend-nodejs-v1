...
// import { TEMP_PROFILE_IMAGES_DIRECTORY_ABSOLUTE_PATH } from '@constants/file-constants'
// import { ERASE_FILES_CONCURRENCY, TEMP_PROFILE_IMAGES_TTL_IN_MS } from '@constants/jobs-configuration-constants'
...
//   try {
//     const entries = await listFiles(TEMP_PROFILE_IMAGES_DIRECTORY_ABSOLUTE_PATH)
//     fileNames = entries.filter((entry) => entry.isFile()).map((file) => file.name)
//   } catch (error) {
//     logger.error({ path: TEMP_PROFILE_IMAGES_DIRECTORY_ABSOLUTE_PATH, error }, LISTING_FILES_ERROR)
//     return false
//   }
